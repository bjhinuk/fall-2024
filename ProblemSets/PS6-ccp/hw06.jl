...
# Assume create_grids.jl is in the same directory
include("create_grids.jl")

# Data loading and preprocessing
function load_and_preprocess_data(url)
    df_bus = CSV.read(HTTP.get(url).body, DataFrame)
    df_bus = @transform(df_bus, :bus_id = 1:nrow(df_bus))
    return df_bus
end

function reshape_data(df_bus)
    dfy = @select(df_bus, :bus_id, Between(:Y1, :Y20), :RouteUsage, :Branded)
    dfy_long = DataFrames.stack(dfy, Not([:bus_id, :RouteUsage, :Branded]))
    rename!(dfy_long, :value => :Y)
    dfy_long = @transform(dfy_long, :time = kron(1:20, fill(1, nrow(df_bus))))
    select!(dfy_long, Not(:variable))

    dfx = @select(df_bus, :bus_id, Between(:Odo1, :Odo20))
    dfx_long = DataFrames.stack(dfx, Not(:bus_id))
    rename!(dfx_long, :value => :Odometer)
    dfx_long = @transform(dfx_long, :time = kron(1:20, fill(1, nrow(df_bus))))
    select!(dfx_long, Not(:variable))

    df_bus_long = leftjoin(dfy_long, dfx_long, on = [:bus_id, :time])
    sort!(df_bus_long, [:bus_id, :time])
    
    return df_bus_long
end

# Flexible logit model estimation (from Question 2)
function estimate_flexible_logit(df_bus_long)
    formula = @formula(Y ~ (Odometer + Odometer^2 + RouteUsage + RouteUsage^2 + Branded + time + time^2)^7)
    model = glm(formula, df_bus_long, Binomial(), LogitLink())
    return model
end

# Part (a): Construct state transition matrices
...
function estimate_structural_parameters(df_bus_long, fvt1)
    ...
    model = glm(@formula(Y ~ Odometer + Branded + fv), df_bus_long, Binomial(), LogitLink())
...
@testset "CCP Estimation Tests" begin
    ...
    @testset "estimate_structural_parameters" begin
        ...
        try
            ...
            num_coefs = length(coef(model))
            @test num_coefs in [3, 4]  # Allow for either 3 or 4 coefficients
            if num_coefs ∉ [3, 4]
                @warn "Unexpected number of coefficients: $num_coefs"
            end
        catch e
